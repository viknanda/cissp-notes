...
Single sign-on allows a user to enter credentials one time and be able to access all resources in primary and secondary network domains. It also allows users to access, through the one set of credentials, all other resources that they have access to, including legacy and other application systems.
...
KDC is single point of failure Secret keys are temporarily stored on users' computers Session keys are decrypted and reside on users' computers SESAME – Secure European System for Applications in a Multi-vendor Environment SESAME is based on symmetric and asymmetric cryptography. Kerberos is strictly symmetric key-based technology.

Kerberos:Tickets as SESAME:Privileged Access Certificates KDC is roughly equivalent to the Privileged Attribute Server A network service is a mechanism that identifies resources on a network and provides a way to make them available to users and programs.
...
A role-based access control (RBAC) model (also called nondiscretionary access control) uses a centrally administered set of controls to determine how subjects and objects interact. Best system for companies that have high employee turnover.

Group – generally a collection of users Role – generally a collection of access rights RBAC model can use role-based access, task-based access or, a lattice-based access.
...
An access control matrix is a table of subjects and objects indicating what actions individual subjects can take upon individual objects. Usually an attribute of DAC models. The access rights can be assigned directly to the subjects (capabilities) or to the objects (ACLs).
...
Separation of duties should be enforced so no one individual can carry out a critical task alone that could prove to be detrimental to the company.
...
Important to differentiate between enticement and entrapment. Entrapment is where the intruder is induced to commit a crime that they weren't originally contemplating. Entrapment is illegal and cannot be used when charging an individual with hacking or unauthorized activity.
